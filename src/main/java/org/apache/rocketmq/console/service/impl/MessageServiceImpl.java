...
public class MessageServiceImpl implements MessageService {
    ...
    public List<MessageView> queryMessageByTopic(String topic, final long begin, final long end) {
       return queryMessageByTopic(topic,begin,end,false);
    }

    @Override
    public List<MessageView> queryMessageByTopic(String topic, final long begin, final long end, final Boolean isCollect) {
            ...
            for (MessageQueue mq : mqs) {
                ...
                for (long offset = minOffset; offset <= maxOffset; ) {
                    try {
                            ...
                            if (!isCollect&&messageViewList.size() > 2000) {
                                break;
                            }

                        PullResult pullResult;

                            pullResult=  consumer.pull(mq, "*", offset, 32);
